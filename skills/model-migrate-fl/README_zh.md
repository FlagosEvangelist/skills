# model-migrate-fl：vLLM 模型迁移技能

## 概述

`model-migrate-fl` 是一个 Claude Code Skill，用于将 **vLLM 上游（upstream）最新版本** 中的模型代码迁移到 **vllm-plugin-FL 项目**（锁定在 vLLM v0.13.0）。
...
│   ├── compatibility-patches.md      # 0.13.0 兼容性补丁目录（P1-P12）
...
#### `SKILL.md`
技能的入口文件。定义了触发条件、参数格式、执行步骤概览、脚本索引和常见问题排查。Claude Code 根据此文件识别和调用技能。
...
## vLLM 与 transformers 的关系

迁移时需理解两者的分工：

```
transformers 提供：              vLLM 提供：
  ✅ Config 解析（AutoConfig）     ✅ 模型结构（自己的 nn.Module）
  ✅ Tokenizer                     ✅ 推理引擎（PagedAttention 等）
  ✅ Image/Video Processor         ✅ 权重加载
  ❌ 不参与推理计算               ✅ API Server
```
